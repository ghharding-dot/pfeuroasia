import { NextResponse } from "next/server";
import { getPartnerContact } from "../../../lib/partnerContacts";
import { hasVaultAccess } from "../../../lib/vaultSession";
import {
  generatePropertyReference,
  normalizeImagePosition,
  normalizePropertyMarket,
  propertyMarketCountry,
  normalizePropertyAccessLevel,
  normalizePropertyVisibility,
  readProperties,
  writeProperties,
  type VaultProperty,
} from "../../../lib/propertyStore";

export async function GET() {
  if (!(await hasVaultAccess())) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  return NextResponse.json(await readProperties());
}

export async function POST(request: Request) {
  if (!(await hasVaultAccess())) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const body = await request.json();
  const properties = await readProperties();
  const now = new Date().toISOString();
  const status: VaultProperty["status"] = body.status === "published" ? "published" : "draft";
  const listingPartner = getPartnerContact(String(body.listingPartnerCode || "DIRECT"));
  const requestedVisibility = normalizePropertyVisibility(body.visibility);
  const accessLevel = normalizePropertyAccessLevel(body.accessLevel, requestedVisibility);
  const visibility: VaultProperty["visibility"] =
    accessLevel === "registered"
      ? "public"
      : requestedVisibility === "public"
        ? "teaser"
        : requestedVisibility;
  const property: VaultProperty = {
    id: crypto.randomUUID(),
    reference: generatePropertyReference(properties),
    title: String(body.title || "").trim(),
    location: String(body.location || "").trim(),
    market: normalizePropertyMarket(body.market),
    country: String(body.country || propertyMarketCountry(normalizePropertyMarket(body.market))).trim().slice(0, 120),
    setting: String(body.setting || "").trim().slice(0, 120),
    views: String(body.views || "").trim().slice(0, 240),
    yearOfConstruction: String(body.yearOfConstruction || "").trim().slice(0, 80),
    developer: String(body.developer || "").trim().slice(0, 180),
    salesAgent: String(body.salesAgent || "").trim().slice(0, 180),
    approximateLocation: String(body.approximateLocation || body.location || "").trim().slice(0, 180),
    price: String(body.price || "").trim(),
    bedrooms: Number(body.bedrooms || 0),
    bedroomsTo: Number(body.bedroomsTo || 0) || undefined,
    bathrooms: Number(body.bathrooms || 0),
    bathroomsTo: Number(body.bathroomsTo || 0) || undefined,
    plotSize: String(body.plotSize || "").trim(),
    builtSize: String(body.builtSize || "").trim(),
    builtSizeTo: String(body.builtSizeTo || "").trim(),
    terraces: String(body.terraces || "").trim(),
    amenities: String(body.amenities || "").trim().slice(0, 1000),
    annualCosts: String(body.annualCosts || "").trim().slice(0, 1000),
    description: String(body.description || "").trim(),
    image: String(body.image || "").trim(),
    secondaryImage: String(body.secondaryImage || "").trim(),
    thirdImage: String(body.thirdImage || "").trim(),
    fourthImage: String(body.fourthImage || "").trim(),
    brochure: String(body.brochure || "").trim(),
    unbrandedBrochure: String(body.unbrandedBrochure || "").trim(),
    adviserName: String(body.adviserName || "PF EuroAsia Property Adviser").trim().slice(0, 120),
    adviserWhatsApp: String(body.adviserWhatsApp || "").trim().slice(0, 40),
    accessLevel,
    visibility,
    publicTitle: String(body.publicTitle || "").trim().slice(0, 120),
    publicLocation: String(body.publicLocation || "").trim().slice(0, 120),
    publicImageApproved: visibility === "confidential" ? false : body.publicImageApproved === true,
    featuredOnHomepage: false,
    homepagePriority: 100,
    imagePosition: normalizeImagePosition(body.imagePosition),
    listingPartnerCode: listingPartner.code,
    listingPartnerName: listingPartner.name,
    submittedBy: "admin",
    approvalStatus: "approved",
    status,
    lastVerifiedAt: status === "published" ? now : undefined,
    createdAt: now,
    updatedAt: now,
  };

  if (!property.title || !property.location || !property.image) {
    return NextResponse.json({ error: "Title, location and main image are required." }, { status: 400 });
  }

  if (property.status === "published" && !property.brochure) {
    return NextResponse.json(
      { error: "Attach one sales brochure PDF before publishing. Save as Draft if the brochure is not ready." },
      { status: 400 },
    );
  }

  properties.unshift(property);
  await writeProperties(properties);
  return NextResponse.json(property, { status: 201 });
}
