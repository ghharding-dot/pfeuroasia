...
async function sendSubmissionEmails(property: VaultProperty, collaboratorEmail: string) {
  ...
  const adminText = [
    ...
    `Market: ${propertyMarketLabel(property.market)}`,
...
export async function POST(request: Request) {
  ...
  const property: VaultProperty = {
    ...
    country: clean(body.country, 120) || propertyMarketCountry(normalizePropertyMarket(body.market)),
    ...
    approximateLocation: clean(body.approximateLocation, 180) || clean(body.location, 180),
    priceAmount,
    ...
    bedroomsTo: listingType === "new-development" ? Number(body.bedroomsTo || 0) || undefined : undefined,
    bathrooms: Number(body.bathrooms || 0),
    bathroomsTo: listingType === "new-development" ? Number(body.bathroomsTo || 0) || undefined : undefined,
    plotSize: clean(body.plotSize, 120),
    builtSize: clean(body.builtSize, 120),
    builtSizeTo: listingType === "new-development" ? clean(body.builtSizeTo, 120) : undefined,
    terraces: clean(body.terraces, 120),
    amenities: clean(body.amenities, 1000),
    ...
    adviserWhatsApp: clean(body.adviserWhatsApp, 40),
    accessLevel,
    visibility,
  ...
  if ((property.market === "malaysia" || property.market === "asia") && !property.country) {
    return NextResponse.json(
